...
struct MenuMan {
    ...
    int choose(N)(int min, N nmax) {
        ...
        if (result != -1)
            while(Keyboard.isKeyPressed(cast(Keyboard.Key)(Keyboard.Key.Num0 + result))) {}

        return result;
    }

	/// set or update menus
    void updateAll(in string project, in string projectLot, in string soundSet, in string[] projectList) {
        updateMain(project, projectLot, soundSet);
    ...
    void updateMain(in string project, in string projectsLot, in string soundSet) {
        ...
        _main = Menu("Welcome to Spell It, - Main Menu -",
            items,
            text("Press a number 1 to ", items.length, " to continue"));
    }
